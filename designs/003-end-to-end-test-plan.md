...
- Each test case that doesn't require seed data uses a **fresh account** (`test+<timestamp>@example.com`).
- Discovery, matching, and connection tests require **seed users** (see below).
...
| Tests | Description |
|-------|-------------|
| §4 Health, §5 Auth, §6 Profile & Handle, §7 Languages, §8 Availability, §9 Discoverable Flag | Each test creates a fresh account (`test+<timestamp>@example.com`). No seed data needed. |
